...
const Listing = () => {
  ...
  useEffect(() => {
    const fetchListing = async () => {
      try {
        ...
        setLoading(false);
        setError(error.message || "Listing fetch failed");
      }
    };
    fetchListing();
  }, [id]);
  ...
  return (
    <main>
      <div className="">
        {error && (
          <p className="text-md font-semibold p-4 text-red-500 text-center">
            {error}
          </p>
        )}
        {loading && (
          <p className="text-xl text-center font-semibold mt-4">Loading...</p>
        )}
                  ...
                  <p className="bg-green-700 text-white px-6 py-3 w-full max-w-[200px] text-center rounded-md">
                    ...
                    {listingData?.discountPrice > 0 &&
                      parseInt(listingData?.regularPrice) -
                        parseInt(listingData?.discountPrice)}{" "}
                    OFF
                  </p>
                )}
              ...
              {currentUser &&
                ...
                !contact && (
                  <button
                    ...
                    className="bg-slate-800 text-white p-3 rounded-lg hover:opacity-90 transition-all duration-300 cursor-pointer disabled:opacity-50"
                  >
                    Contact Landlord
                  </button>
                )}
